...
ALTER TABLE presensi_records DISABLE ROW LEVEL SECURITY;</pre>
        ...
        <div class="max-w-7xl mx-auto px-4 mt-6">
            <div id="app-grid" class="grid-container">
                <!-- Diisi secara dinamis melalui init() -->
            </div>
    ...
    <script>
        ...
        // Pemicu Rahasia 1: Triple Click pada Judul Header Utama
        ...
        // Pemicu Rahasia 2: Shortcut Keyboard Ctrl + Shift + X
        ...
        function setSyncStatus(status, text) {
            ...
            if (status === 'online') {
                ...
                if (dbLabel) dbLabel.innerText = "Penyimpanan Lokal";
            }
        }
        ...
        function showToast(message, type = 'success') {
            ...
            let icon = 'info';
            if (type === 'success') { icon = 'check-circle'; card.style.borderLeftColor = 'var(--success)'; } 
            else if (type === 'error') { icon = 'alert-octagon'; card.style.borderLeftColor = 'var(--danger)'; } 
            else if (type === 'warning') { icon = 'alert-triangle'; card.style.borderLeftColor = 'var(--warning)'; }
        ...
        function pindahkanSiswaKelas() {
            ...
            if (targetSiswa === "ALL") {
                ...
                customConfirm("Naik Kelas Massal", `Pindahkan seluruh (${totalSiswa}) siswa dari ${klsAsal} ke ${klsTujuan}?`, async () => {
                    saveCurrentSelections();
                ...
                customConfirm("Pindah Kelas Siswa", `Pindahkan ${targetSiswa} dari ${klsAsal} ke ${klsTujuan}?`, async () => {
                    saveCurrentSelections();
        ...
        async function tambahKelasBaru() {
            ...
            if (!klsBaru) { showToast("Nama kelas tidak boleh kosong!", "error"); return; }
            if (dataSiswa[klsBaru]) { showToast("Kelas tersebut sudah terdaftar!", "warning"); return; }
        ...
        async function tambahSiswaBaru() {
            ...
            if (!kls) { showToast("Silakan buat kelas terlebih dahulu!", "warning"); return; }
            if (!namaSiswa) { showToast("Nama siswa tidak boleh kosong!", "error"); return; }
            if (dataSiswa[kls].includes(namaSiswa)) { showToast("Nama siswa sudah ada di kelas tersebut!", "warning"); return; }
        ...
        function hapusKelas(kls) {
            if (isAppDisabled) return;
            customConfirm("Hapus Kelas", `Apakah Anda yakin ingin menghapus ${kls} beserta seluruh siswanya?`, async () => {
                saveCurrentSelections();
        ...
        async function simpanData() {
            ...
            if (isCloudActive && supabaseClient) {
                try {
                    await supabaseClient.from('presensi_records').insert({
                        id: payload.id,
                    ...
                    showToast("Presensi berhasil disimpan secara real-time ke Supabase!", "success");
                    ...
                    await supabaseClient.from('presensi_data').upsert({
                        key: 'current_session',
                        value: { date: new Date().toDateString(), selections: {} }
                    });
        ...
        function cetak(payload, orientation = 'p') {
            if (isAppDisabled) return;
            try {
                ...
                doc.setFontSize(11);
                doc.text("LAPORAN PRESENSI DIGITAL SANTRI", doc.internal.pageSize.getWidth() / 2, 58, { align: 'center' });
                doc.setFontSize(9);
                ...
                doc.save(`Presensi_HQ_${payload.waktuSimpan.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`);
        ...
        function hapusLaporan(id) {
            ...
            customConfirm("Hapus Laporan", "Apakah Anda yakin ingin menghapus arsip laporan ini?", async () => {
                ...
                if (isCloudActive && supabaseClient) {
                    try {
                        await supabaseClient.from('presensi_records').delete().eq('id', id);
                    } catch (e) {}
                }
                localStorage.setItem(DB_KEY, JSON.stringify(localDb));
                renderHistori();
        ...
        function hitungStatistikDisiplin(hariMundur = 7) {
            ...
            let totalHadirGlobal = 0, totalTerdaftarGlobal = 0;
            let akumulasiAman = 0, akumulasiTerlambat = 0, akumulasiPeringatan = 0, akumulasiSakit = 0, akumulasiIzin = 0, akumulasiAlpa = 0;
            ...
            Object.entries(dataSiswa).forEach(([kelas, listSiswa]) => {
                listSiswa.forEach(nama => {
                    studentMap[nama] = {
                        nama, kelas, aman: 0, terlambat: 0, peringatan: 0, sakit: 0, izin: 0, alpa: 0, totalSesi: 0
                    };
                });
            });
            ...
            sesiValid.forEach(sesi => {
                ...
                sesi.data.forEach(siswa => {
                    if (!studentMap[siswa.nama]) {
                        studentMap[siswa.nama] = { nama: siswa.nama, kelas: siswa.kelas, aman: 0, terlambat: 0, peringatan: 0, sakit: 0, izin: 0, alpa: 0, totalSesi: 0 };
                    }

                    const profile = studentMap[siswa.nama];
                    profile.totalSesi++;

                    if (siswa.status === "AMAN") { profile.aman++; akumulasiAman++; }
                    else if (siswa.status === "TERLAMBAT") { profile.terlambat++; akumulasiTerlambat++; }
                    else if (siswa.status === "PERINGATAN") { profile.peringatan++; akumulasiPeringatan++; }
                    else if (siswa.status === "SAKIT") { profile.sakit++; akumulasiSakit++; }
                    else if (siswa.status === "IZIN") { profile.izin++; akumulasiIzin++; }
                    else { profile.alpa++; akumulasiAlpa++; }
                });
            });
